...
class LidarBridge(Node):
    def __init__(self) -> None:
        ...
        # RELIABLE in sim: the host's 212 KB UDP buffers drop fragments of the
        # ~740 KB sim clouds and best-effort readers then lose every sample.
        ...
        sensor_qos = QoSProfile(
            reliability=(QoSReliabilityPolicy.RELIABLE if self.get_parameter("reliable_qos").value
        ...
        if self._tf_buf is not None:
            try:
                ...
                try:
                    ...
                    # Isaac Sim stamps the cloud ~1 us after the matching /tf
                    # (float rounding) - use the latest transform instead.
                    transform = self._tf_buf.lookup_transform(
